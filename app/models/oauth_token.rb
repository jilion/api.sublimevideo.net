require 'app/models/client_application'
...
class OauthToken < ActiveRecord::Base
  ...
  scope :valid, -> { where(invalidated_at: nil).where('authorized_at IS NOT NULL') }
  ...
  def generate_keys
    self.token  = SecureRandom.base64(40).gsub(/\W/, '')[0, 40]
